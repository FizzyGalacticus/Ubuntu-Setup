...
then
	echo "64bit Detected" > /dev/null &&
	echo "Installing Qt" > /dev/null &&
	...
	rm -f google-chrome-stable_current_amd64.deb > /dev/null &&
else
	echo "32bit Detected" > /dev/null &&
	echo "Installing Qt" > /dev/null &&
	...
	rm -f google-chrome-stable_current_i386.deb > /dev/null &&
fi
